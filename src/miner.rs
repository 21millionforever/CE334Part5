    ...
    fn miner_loop(&mut self) {
        ...
        loop {
            ...
            if let OperatingState::Run(i) = self.operating_state {
                ...
                let mut transactions: Vec<RawTransaction> = vec![Default::default()];
                ...
                if block.hash() <= difficulty {
                    ...
                    let new_transaction_hashes: Vec<H256> = transactions.iter().map(|tx| tx.hash()).collect();
                    self.server.broadcast(Message::NewTransactionHashes(new_transaction_hashes));
                }
            }
        }
    }
